...
class TableTarget(Target):
    ...
    def __init__(self, task, columntables, timespan=None, bounds=None,
        ...
        table_id = '"{schema}".{tablename}'.format(schema=schema,
                                                   tablename=tablename)

        columns = []

        with session_scope() as session:
            existing_table = session.query(BMDTable).get(table_id)
            if existing_table:
        ...
        with session_scope() as session:
            session.execute('CREATE SCHEMA IF NOT EXISTS "{schema}"'.format(
                schema=schema))
            self.metatable = BMDTable(id=table_id, tablename=slug_column(table_id),
                                      bounds=bounds, timespan=timespan,
