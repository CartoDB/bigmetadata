...
def remove_from_do(id):
    ...
    numtables = session.execute("SELECT count(*) FROM observatory.obs_table WHERE id LIKE '{}%'".format(id)).fetchone()[0]
    ...
    for table in session.execute("SELECT id FROM observatory.obs_table WHERE id LIKE '{}%'".format(id)).fetchall():
        print "\t" + table[0]
    numcols = session.execute("SELECT count(*) FROM observatory.obs_column WHERE id LIKE '{}%'".format(id)).fetchone()[0]
    print " --> This will delete {} entries from observatory.OBS_COLUMNS" \
        .format(numcols)
    for column in session.execute("SELECT id FROM observatory.obs_column WHERE id LIKE '{}%'".format(id)).fetchmany(MAX_COLS_TO_PRINT):
        print "\t" + column[0]
    ...
    print " --> This will drop {} tables from the 'observatory' schema" \
        .format(session.execute("SELECT count(*) FROM observatory.obs_table WHERE id LIKE '{}%'".format(id)).fetchone()[0])
    yn = raw_input("Continue? (Y/N) ")

    if yn != "Y":
        return

    for table in session.execute("SELECT tablename FROM observatory.obs_table WHERE id LIKE '{}%'".format(id)).fetchall():
