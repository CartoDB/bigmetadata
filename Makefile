sh:
	docker-compose -p $$(whoami) run --rm bigmetadata /bin/bash

extension-perftest: extension
	docker-compose -p $$(whoami) run --rm bigmetadata nosetests -s observatory-extension/src/python/test/perftest.py

extension-perftest-record: extension
	mkdir -p perftest
	docker-compose -p $$(whoami) run --rm \
...
extension-autotest: extension
	docker-compose -p $$(whoami) run --rm bigmetadata nosetests observatory-extension/src/python/test/autotest.py

test: extension-perftest extension-autotest

python:
	docker-compose -p $$(whoami) run --rm bigmetadata python

build:
	docker-compose -p $$(whoami) build

build-postgres:
	docker build -t recessionporn/bigmetadata_postgres:latest postgres

psql:
	docker-compose -p $$(whoami) run --rm bigmetadata psql

acs:
	docker-compose -p $$(whoami) run --rm bigmetadata luigi \
...
kill:
	docker-compose -p $$(whoami) ps | grep _run_ | cut -c 1-34 | xargs docker stop
...
run:
	docker-compose -p $$(whoami) run --rm bigmetadata luigi --local-scheduler --module tasks.$(RUN_ARGS)

dump: test
	docker-compose -p $$(whoami) run --rm bigmetadata luigi --module tasks.carto DumpS3

# update the observatory-extension in our DB container
# Depends on having an observatory-extension folder linked
extension:
	docker exec $$(docker-compose -p $$(whoami) ps -q postgres) sh -c 'cd observatory-extension && make install'
...
dataservices-api: extension
	docker exec $$(docker-compose -p $$(whoami) ps -q postgres) sh -c ' \
	...
	docker-compose -p $$(whoami) run --rm bigmetadata psql -f /bigmetadata/postgres/dataservices_config.sql
	docker exec $$(docker-compose -p $$(whoami) ps -q redis) sh -c \
...
sh-sql:
	docker exec -it $$(docker-compose -p $$(whoami) ps -q postgres) /bin/bash

py-sql:
	docker exec -it $$(docker-compose -p $$(whoami) ps -q postgres) python

# Regenerate fixtures for the extension
extension-fixtures:
	docker-compose -p $$(whoami) run --rm bigmetadata \
	  python observatory-extension/scripts/generate_fixtures.py

extension-unittest:
	docker exec -it \
	  $$(docker-compose -p $$(whoami) ps -q postgres) \
...
etl-unittest:
	docker-compose -p $$(whoami) run --rm bigmetadata /bin/bash -c \
...
restore:
	docker-compose -p $$(whoami) run --rm -d bigmetadata pg_restore -U docker -j4 -O -x -e -d gis $(RUN_ARGS)

docs:
	docker-compose -p $$(whoami) run --rm bigmetadata /bin/bash -c 'cd docs && make html'
...
ps:
	docker-compose -p $$(whoami) ps

stop:
	docker-compose -p $$(whoami) stop

up:
	docker-compose -p $$(whoami) up -d
